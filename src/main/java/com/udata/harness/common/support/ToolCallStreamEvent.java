...
public class ToolCallStreamEvent extends AbsAgentEvent {
    /**
     * 底层工具调用事件类型（START / ARGS_DELTA / END）。
     */
    private final ChatEventType eventType;

    /**
     * 一次流式工具调用在前端的稳定标识（响应 Id:步骤:索引）。
     */
     ...
     *
     * @param trace 当前推理轨迹，提供 runId、agentName 与会话上下文
     * @param event 底层聊天事件
     */
     ...
     * 返回底层工具调用事件类型。
     *
     * @return 事件类型（START / ARGS_DELTA / END）
     */
     ...
     * 返回工具名称。
     *
     * @return 工具名；部分参数分片可能不携带名称时返回 null
     */
